...
export const SHOP_LOCATION = {
  lat: 10.762622,
  lng: 106.660172,
  name: 'Quán Nem (vị trí mẫu — cần đổi lại)',
}
...
export function calcShippingFee(distanceKm) {
  ...
  if (distanceKm <= 5) return 5000 + Math.ceil(distanceKm - 1) * 3000
  if (distanceKm <= 15) return 15000 + Math.ceil(distanceKm - 5) * 4000
  return 55000 + Math.ceil(distanceKm - 15) * 5000
}

// Định vị địa chỉ chữ thành tọa độ — gọi qua API route của chính web
// (route đó mới là nơi thực sự gọi OpenStreetMap, để tránh bị chặn từ trình duyệt)
